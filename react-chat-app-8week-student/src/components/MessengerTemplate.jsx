...
function MessenagerTemplate({ getChatLog }) {
  ...
  const onMsgSubmit = (e) => {
    ...
    // 8주차 실습 구현
    console.log(e);
    if (MsgState.username === '' || MsgState.message === '') {
      alert("메시지를 입력하세요");
      return;
    }
  ...
  return (
    <Form onSubmit={onMsgSubmit}>
      ...
      <UserName>
        User Name
        <br ></br>
        <TextField onChange={onMsgChange} name="username" value={MsgState.username}></TextField>
      </UserName>
      <MessageContents>
        Message
        <br ></br>
        <TextField onChange={onMsgChange} name="message" value={MsgState.message}></TextField>
      </MessageContents>
      <Button onClick={onMsgSubmit}>Send</Button>
    </Form>
  );
}
