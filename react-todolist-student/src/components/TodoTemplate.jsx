...
function TodoTemplate() {
  ...
  const onRemoveDoneTodo = (id) => {
    // 7주차 실습 구현
    // 주어진 Done List 항목(체크된 항목)의 id로 Done List에서 해당 항목을 찾고
    const deleteTodo = doneTodos.find((todo) => todo.id === id);
    // 삭제된 todo -> deletedTodos에 저장
    setDeletedTodos([...deletedTodos, deleteTodo]);
    // filter 함수 사용해서 id에 해당하는 항목만 todos 배열에서 제외
  ...
  const onToggleToDo = (id) => {
    // 7주차 실습 구현
    const willDo = todos.find((todo) => todo.id === id);
    willDo.checked = true;
    // 삭제된 willDo -> deletedTodos에 저장
    setDoneTodos([...doneTodos, willDo]);
    // filter 함수 사용해서 id에 해당하는 항목만 what to do 배열에서 제외
    setTodos(todos.filter((todo) => todo.id !== id));
  };
  ...
  const onToggleDoneToDo = (id) => {
    // 7주차 실습 구현
    // 주어진 Done List 안 항목을 찾아 항목 객체의 속성을 사용하여 체크 해제한 후
    const Done = doneTodos.find((todo) => todo.id === id);
    Done.checked = false;
    //What To Do에 추가하고 
    setTodos([...todos, Done])
    // Done List에서 제외
